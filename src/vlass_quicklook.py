...
def search_tiles(tiles_df, coord, verbosity=0, mode='all'):
    ...
    if tile.shape[0] > 1:
        ...
        if mode == 'recent':
            ...
            # download other epochs except for the most recent one
            tile = tile.loc[[tile.index[:-1]]]
            if verbosity > 1:
                print('[INFO] Returning all tiles EXCEPT for the most recent observation:')
                print('{}'.format(tile.loc[:, 'name']))
...
def get_closest_subtile_url(tile, coord, verbosity=0):
    ...
    for idx in tile.index:
        ...
        if epoch=='VLASS1.1' or epoch=='VLASS1.2':
             # Naming changes on VLASS server
            epoch = epoch+'v2'
        ...
        if (len(subtile_name)>1):
            for i in range(len(subtile_name)):
                url_base = 'https://archive-new.nrao.edu' \
                    '/vlass/quicklook/{:s}/{:s}/'.format(epoch, name)
                url_base = url_base + '/{}/'.format(subtile_name[subtile_name.index[i]])

                image_name = '{}.I.iter1.image.pbcor.tt0.subim.fits'.format(subtile_name[subtile_name.index[i]][ 0:-1])
            ...
            url_base = url_base + '/{}/'.format(subtile_name[subtile_name.index[0]])

            image_name = '{}.I.iter1.image.pbcor.tt0.subim.fits'.format(subtile_name[subtile_name.index[0]][0:-1])
            image_name_list.append(image_name)
            epoch_list.append(epoch)

            vlass_urls.append(url_base+image_name)
        
    return vlass_urls, epoch_list, image_name_list


def search_vlass_quicklook(coord, update_summary=False,
                           mode='all',verbosity=0):
...
def get_quicklook_url(ra, dec,
    ...
    if tile is not None:
        vlass_url, epoch, raw_file_name_list = get_closest_subtile_url(tile, coord, verbosity=verbosity)
        return vlass_url, epoch, raw_file_name_list
    else:
...
def make_vlass_cutout(ra_deg, dec_deg, fov, raw_image_name, image_name,
    ...
    filename = image_folder_path + '/' + raw_image_name
