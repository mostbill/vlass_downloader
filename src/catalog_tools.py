...
import utils as ut
import vlass_quicklook as vlass_quicklook
...
def convert_urltable_to_pandas(data, sep=',', header=0, skip_header=1,
    ...
    if data_string[0] == 'no rows found':
        ...
        for dat in data_string[skip_header:-skip_footer]:
            ...
            df = pd.concat([df, pd.Series(dat.split(sep),
...
def get_photometry(table, ra_col_name, dec_col_name, object_col_name, surveys, bands, image_folder_path,
                   fovs, verbosity=0):
    ...
    for jdx, band in enumerate(bands):
        ...
        for idx in table.index:
            ...
            if object_col_name is not None:
                obj_name = table.loc[idx, object_col_name]
                # use user provided obj name
                table.loc[:, 'temp_object_name'] = str(obj_name)
            ...
            if survey == "ps1" and band in ["g", "r", "i", "z", "y"]:
            ...
            elif survey in vsa_survey_list and band in ['Z', 'Y', 'J', 'H', 'Ks']:
            ...
            elif survey == 'decals' and band in ['g', 'r', 'z']:
                ...
                if file_exists is not True:
                    url = get_decals_image_url(ra, dec, fov, band,
                                              verbosity=verbosity)
                ...
                vlass_img_name = table.temp_object_name[idx]
                ...
                #raw_img_name = table.temp_object_name[idx]+'_raw'

                #raw_file_path = image_folder_path + '/' + raw_img_name + '_raw.fits'
                
                if(len(raw_img_name_list)>1):
                    raw_img_names_temp=[] # for later cutout use
                    _list_raw_file_exists=[]
                    for raw_img_name in raw_img_name_list:
                        raw_img_names_temp.append(raw_img_name)
                        raw_file_path = image_folder_path + '/' + raw_img_name
                        _raw_file_exists = os.path.isfile(raw_file_path)
                        _list_raw_file_exists.append(_raw_file_exists)
                        #embed()
                        
                    raw_file_exists=all(_list_raw_file_exists)
                else:
                    raw_file_path = image_folder_path + '/' + raw_img_name_list[0]
                    ...
                    try:
                        ...
                        if(len(url)>1):
                            ...
                            for i in range(len(url)):
                                #embed()
                                if(_list_raw_file_exists[i]==True):
                                    continue # already downloaded, skip
                                #raw_img_name_temp=raw_img_name+'_'+epoch[i]
                                raw_img_name_temp=raw_img_name_list[i]
                                raw_img_names_temp.append(raw_img_name_temp)
                                download_image(url[i], raw_img_name_temp, image_folder_path,
                                    survey=survey, verbosity=verbosity)
                        else:      
                            # single epoch source
                            #raw_img_name_temp=raw_img_name+'_'+epoch[0]
                            raw_img_name_temp=raw_img_name
                ...
                if file_exists is False:
                    ...
                    try:
                        if(len(url)>1):
                            ...
                            for i in range(len(url)):
                                print(epoch[i])
                                if 'VLASS1' in epoch[i]:
                                    epoch[i]='EP01'
                                if 'VLASS2' in epoch[i]:
                                    epoch[i]='EP02'
...
def download_image(url, image_name, image_folder_path, survey, verbosity=0):
    ...
    try:
        ...
        if check_ok:
            ...
            if survey in unzipped_download_list:
                ...
                file = datafile.read()
                output = open(image_folder_path+'/'+image_name, 'wb')
                output.write(file)
                output.close()
                if verbosity > 0:
                    print("Download of {} to {} completed".format(image_name,
...
def get_decals_image_url(ra, dec, fov, band, verbosity=0):
    
    size = int(fov/0.30) # where 0.27 is the pixel scale for the highest possible resolution, here size in units of pixels
    
    url = ("https://www.legacysurvey.org/viewer/fits-cutout?"+\
        "ra={ra}&dec={dec}&size={size}&layer=ls-dr9&pixscale=0.30&bands={band}").format(**locals())
...
def get_skymapper_deepest_image_url(ra, dec, fov, band, verbosity=0):
    ...
    # exposure times
    def return_sorted_size_images(df):
        # return the sorted image series
        for i in range(len(df)):
            # size string -> int
            df['size'][i] = df['size'][i].replace('(','').replace(')','').replace(' ', '').split(',')

        def see_size_picture(x):
            # use this to determine size difference in X and Y
            index = []
            for i in range(len(x)):
                try:
                    index.append(abs(int(x[i][0])-int(x[i][1])))
                except:
                    index.append(-99)
                    
            return index
        
        _index = see_size_picture(df['size'])

        df['size_index'] = _index

        return df

    if band in ['i', 'z']:
        image_df.query('image_type == "main"', inplace=True) # must be main ( in the catalogue)
        image_df = image_df.loc[image_df['exptime'] == image_df['exptime'].max()] # the highest exptime, but maybe multiple
        image_df = image_df.reset_index(drop=True)
        image_df.sort_values(['mjd_obs'], inplace=True) # most recent
        image_df = return_sorted_size_images(image_df)
        image_df.sort_values(['size_index'], inplace=True) # size is square
    
    else:
        # images except for i, z anyway, just for visual inspection
        image_df = image_df.loc[image_df['exptime'] == image_df['exptime'].max()]
        image_df = image_df.reset_index(drop=True)
        image_df.sort_values(['mjd_obs'], inplace=True)
        image_df = return_sorted_size_images(image_df)
        image_df.sort_values(['size_index'], inplace=True)
        
    if image_df.shape[0] > 0:
        return image_df.loc[image_df.index[0], 'get_fits']
    else:
        return None
...
def get_skymapper_mag_cat(ra, dec, band):
    """
    Return the magnitude and error in the catalogue
    """

    url_base = 'http://skymapper.anu.edu.au/sm-cone/public/query?'
    skymapper_cat_url = url_base + 'RA={}&DEC={}&SR=0.000556' # SR is distance in degrees

    skymapper_cat_url += '&VERB=3&RESPONSEFORMAT=CSV'

    cat_df = pd.read_cv(urlopen(skymapper_cat_url))
    cat_band_mag = str(band) + '_psf'
    cat_band_mag_err = 'e_' + cat_band_mag

    if cat_df.shape[0] > 0:
        return cat_df.loc[cat_band_mag], cat_df.loc[cat_band_mag_err]
    else:
        return None, None

    # For every source, there is only one mean magnitude
...
def get_unwise_image_url(ra, dec, npix, band, data_release, filetype="image"):
    ...
    :param data_release: str
        ...
        neo2, neo3, neo4, neo5, neo6, neo7, allwise
    :param verbosity: int
        Verbosity > 0 will print verbose statements during the execution
    :return: str
        Returns the url to the DES DR1 image cutout
